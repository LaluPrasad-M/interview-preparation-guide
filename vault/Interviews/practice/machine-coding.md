...
| P1 | Rate limiter | allow max N requests per M seconds | token bucket or sliding window log. Token bucket stores `[tokens_left, last_updated_timestamp]`. Sliding window uses a queue of timestamps, popping ones that fall outside the window. Both written out in [[rate-limiter]] |
...
| P2 | Leaderboard | millions of players, constant score updates, fetch top 10 and player rank | [[skip-list|skip list]] or TreeMap plus a hash map. The hash map stores player ID to node. A skip list or self balancing BST keeps scores sorted, allowing logarithmic updates and rank lookups |
