...
**2. Low latency task queues.** Commonly used for background jobs, async processing and task queues. A user uploads an image, a thumbnail job is queued, a worker processes the image.
...
**1. Massive throughput.** Millions of events per second and distributed streaming. Excellent for logs, telemetry, clickstreams and analytics.

**2. Event streaming architecture.** Ideal when the events themselves are valuable. `OrderCreated`, `PaymentProcessed`, `InventoryUpdated` become permanent history.
...
**4. Event sourcing and CQRS.** Excellent for rebuilding state, replaying events and audit trails. Huge in fintech.
